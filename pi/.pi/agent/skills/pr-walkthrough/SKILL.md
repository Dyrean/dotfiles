---
name: pr-walkthrough
description: Generate an interactive visual walkthrough of any GitHub pull request as a local HTML webapp. Produces a multi-slide presentation with SVG diagrams, annotated code, and architecture visuals. Pass a PR URL and optional audience context (e.g. "assume I don't know Rust"). Use when the user says "walkthrough this PR", "visual walkthrough", "PR presentation", or provides a GitHub PR URL and asks for a walkthrough.
---

# PR Walkthrough

Generate a polished, interactive HTML slide deck that walks through a GitHub pull request. The output is a self-contained HTML file with no external dependencies that can be opened locally or served if the user explicitly asks for that.

## Input

The user provides a GitHub PR URL and optionally additional context after it. Examples:
...
Fetch PR metadata + diff from GitHub:
...
If the user provides a full URL, parse out the PR number and repo. If needed, set the repo context:
```bash
...
`.agents/session/pr-walkthrough/<slug>-<timestamp>/index.html`

Use a short lowercase kebab-case slug derived from the PR or topic, and use a sortable timestamp format like `YYYY-MM-DD-HHMM`.

Use the [HTML template reference](references/html-template.md) for the exact CSS framework, component library, and JavaScript.
...
### 5. Serve (only if explicitly requested)

If the user asks to host/serve/share the walkthrough:

```bash
# Start local HTTP server
cd .agents/session/pr-walkthrough/<slug>-<timestamp> && python3 -m http.server 8787 &>/tmp/walkthrough-server.log &
echo "Server PID: $!"
sleep 1
```
...
- Prefer giving the user the local file path: `.agents/session/pr-walkthrough/<slug>-<timestamp>/index.html`
- If they want local browser launch on Linux, use `xdg-open .agents/session/pr-walkthrough/<slug>-<timestamp>/index.html`
- Only start a local server when the user asks for serving/sharing

If a server was started, provide:

- the local URL (`http://localhost:8787`)
- the teardown command: `kill <server_pid>`
